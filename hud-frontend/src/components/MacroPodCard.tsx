...
export const MacroPodCard: React.FC<Props> = ({ pod }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            {pod.metrics.map(metric => (
              <div key={metric.ticker} className="flex items-center justify-between p-3 rounded-md bg-secondary/20">
                ...
                <div className="flex-1 text-center">
                  ...
                  <div className="text-base font-bold text-primary">{(metric.historicalPercentile ?? 0).toFixed(1)}th</div>
                </div>
                <div className="flex-1 text-right flex flex-col items-end">
                  <div className="text-lg font-mono font-bold">{(metric.currentValue ?? 0).toFixed(2)}</div>
                  <Badge variant={(metric.changePercent ?? 0) >= 0 ? "secondary" : "destructive"} className={`mt-1 text-[10px] font-mono font-bold px-1.5 py-0 ${(metric.changePercent ?? 0) >= 0 ? 'bg-success/20 text-success border-success/40' : 'bg-destructive/20 text-destructive border-destructive/40'}`}>
                    {(metric.changePercent ?? 0) >= 0 ? '+' : ''}{(metric.changePercent ?? 0).toFixed(2)}%
                  </Badge>
                </div>
              </div>
            ))}
      ...
      <Card className="h-full flex flex-col border-border/60 bg-card/60">
        <CardHeader className="pb-3 border-b border-border/50">
          <CardTitle className="text-lg flex items-center gap-2">
